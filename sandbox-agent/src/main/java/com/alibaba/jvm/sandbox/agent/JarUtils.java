...
public class JarUtils {
    ...
    private static SoftReference<Map<String, JarFile>> jarFileCache;
    private static SoftReference<Map<String, File>> fileCache;

    static {
        jarFileCache = new SoftReference<>(new ConcurrentHashMap<>());
    ...
    static File findFile(File agentJar, String fileName) throws IOException {
        StringSequence spec = new StringSequence(fileName);
        Map<String, File> cache = fileCache.get();
        File file = (cache != null) ? cache.get(spec.toString()) : null;
    ...
    static void addToRootFileCache(String fileName, JarFile jarFile) {
        ...
        if (cache == null) {
            ...
            jarFileCache = new SoftReference<>(cache);
        }
        cache.put(fileName, jarFile);
    }

    static void addToRootFileCache(String fileName, File file) {
        Map<String, File> cache = fileCache.get();
        if (cache == null) {
            cache = new ConcurrentHashMap<>(8);
